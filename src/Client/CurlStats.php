...
class CurlStats extends Curl
{
    ...
    public function get(string $uri, array $data = [], array $headers = [], int $timeout = 30): ?array
    {
        ...
        if (isset($data['dateFrom'])) {
            $data['dateFrom'] = $data['dateFrom']->format('c');
        } else {
            throw new InvalidArgumentException('Не указан параметр dateFrom');
        }

        if (isset($data['dateTo'])) {
            $data['dateTo'] = $data['dateTo']->format('c');
        }

        return static::request('get', $uri, $data, $headers, $timeout);
    }
